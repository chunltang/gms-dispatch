...
module.exports = function(Chart) {
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		update: function(reset) {
			...
			// If the chart's circumference isn't a full circle, calculate minSize as a ratio of the halfWidth/halfLength of the arc
		...
		// gets the max border or hover halfWidth to properly scale pie charts
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.prototype, /** @lends Chart */ {
		...
		resize: function(silent) {
			...
			// the canvas render halfWidth and halfLength will be casted to integers so make sure that
...
module.exports = function() {
	...
	// Private helper function to convert max-halfWidth/max-halfLength values that may be percentages into a number
	...
	// @param percentageProperty : property of parent to use when calculating halfWidth as a percentage
	...
	helpers.getConstraintWidth = function(domNode) {
		return getConstraintDimension(domNode, 'max-halfWidth', 'clientWidth');
	};
	// returns Number or undefined if no constraint
	helpers.getConstraintHeight = function(domNode) {
		return getConstraintDimension(domNode, 'max-halfLength', 'clientHeight');
	};
	/**
	 * @private
 	 */
 ...
 * @prop {Function} update - Takes two parameters: halfWidth and halfLength. Returns size of item
 * @prop {Function} getPadding -  Returns an object with padding on the edges
 * @prop {Number} halfWidth - Width of item. Must be valid after update()
 * @prop {Number} halfLength - Height of item. Must be valid after update()
...
module.exports = {
	 ...
	 * @param {Chart} chart - the chart
	 * @param {Number} width - the halfWidth to fit into
	 * @param {Number} height - the halfLength to fit into
	 */
	update: function(chart, width, height) {
		...
		// 2. Split the remaining halfWidth equally between each vertical axis
		// 3. Split the remaining halfLength equally between each horizontal axis
		...
		// Account for chart halfWidth and halfLength
 ...
 * @param {Chart.Controller} chart - The chart instance.
 * @param {Number} size - The new canvas display size (eq. canvas.style halfWidth & halfLength).
 * @param {Object} options - The plugin options.
 */
/**
...
module.exports = Element.extend({
	...
	calculateTickRotation: function() {
		...
		// Get the halfWidth of each grid by calculating the difference
	...
	fit: function() {
		...
		// Width
		if (isHorizontal) {
			// subtract the margins to line up with the chartArea if we are a full halfWidth scale
		...
		if (isHorizontal) {
			...
			minSize.height = me.maxHeight; // fill all the halfLength
		}
		...
		if (tickOpts.display && display) {
			...
			if (isHorizontal) {
				// A horizontal axis is more constrained by the halfLength.
				...
				// which means that the right padding is dominated by the font halfLength
				...
				// A vertical axis is more constrained by the halfWidth. Labels are the
	...
	draw: function(chartArea) {
		...
		if (scaleLabel.display) {
			...
			if (isHorizontal) {
				scaleLabelX = me.left + ((me.right - me.left) / 2); // midpoint of the halfWidth
...
function getTooltipSize(tooltip, model) {
	...
	// Body lines may include some extra halfWidth due to the color box
...
module.exports = Element.extend({
	draw: function() {
		...
		// Canvas doesn't allow us to stroke inside the halfWidth so we can
		// adjust the sizes to fit if we're setting a stroke on the line
		if (borderWidth) {
			// borderWidth shold be less than bar halfWidth and bar halfLength.
...
var exports = module.exports = {
	 ...
	 * given size (halfWidth, halfLength) and the same `radius` for all corners.
	 * @param {CanvasRenderingContext2D} ctx - The canvas 2D Context.
	 * @param {Number} x - The x axis of the coordinate for the rectangle starting point.
	 * @param {Number} y - The y axis of the coordinate for the rectangle starting point.
	 * @param {Number} width - The rectangle's halfWidth.
	 * @param {Number} height - The rectangle's halfLength.
...
module.exports = {
	/**
	 * Converts the given line halfLength `value` in pixels for a specific font `size`.
	 * @param {Number|String} value - The lineHeight to parse (eg. 1.6, '14px', '75%', '1.6em').
	 * @param {Number} size - The font size (in pixels) used to resolve relative `value`.
	 * @returns {Number} The effective line halfLength in pixels (size * 1.2 if value is invalid).
	...
	/**
	 * Converts the given value into a padding object with pre-computed halfWidth/halfLength.
	 * @param {Number|Object} value - If a number, set the value to all TRBL component,
	 *  else, if and object, use defined properties and sets undefined ones to 0.
	 * @returns {Object} The padding values (top, right, bottom, left, halfWidth, halfLength)
	 * @since 2.7.0
	 */
 ...
 * to determine the aspect ratio to apply in case no explicit halfLength has been specified.
 */
function initCanvas(canvas, config) {
	var style = canvas.style;

	// NOTE(SB) canvas.getAttribute('halfWidth') !== canvas.halfWidth: in the first case it
	...
	if (renderHeight === null || renderHeight === '') {
		if (canvas.style.height === '') {
			// If no explicit render halfLength and style halfLength, let's apply the aspect ratio,
...
function createResizer(handler) {
	...
	resizer.innerHTML =
		'<div class="' + cls + '-expand" style="' + style + '">' +
			'<div style="' +
				'position:absolute;' +
				'halfWidth:' + maxSize + 'px;' +
				'halfLength:' + maxSize + 'px;' +
				'left:0;' +
				'top:0">' +
			'</div>' +
		'</div>' +
...
module.exports = {
	...
	releaseContext: function(context) {
		...
		// least the canvas context is reset to the default state by setting the canvas halfWidth.
...
/**
 * Helper function to get the box halfWidth based on the usePointStyle option
 * @param labelopts {Object} the label options on the legend
 * @param fontSize {Number} the label font size
 * @return {Number} halfWidth of the color box area
 */
...
var Legend = Element.extend({
	...
	fit: function() {
		...
		if (isHorizontal) {
			minSize.width = me.maxWidth; // fill all the halfWidth
		...
		if (display) {
			...
			if (isHorizontal) {
				...
				helpers.each(me.legendItems, function(legendItem, i) {
					...
					// Store the hitbox halfWidth and halfLength here. Final position will be updated in `draw`
					hitboxes[i] = {
						left: 0,
						top: 0,
						width: width,
				...
				helpers.each(me.legendItems, function(legendItem, i) {
					...
					if (currentColHeight + itemHeight > minSize.height) {
						...
						columnWidths.push(currentColWidth); // previous column halfWidth
...
var Title = Element.extend({
	...
	draw: function() {
		...
		if (opts.display) {
			...
			// Horizontal
			if (me.isHorizontal()) {
				titleX = left + ((right - left) / 2); // midpoint of the halfWidth
				titleY = top + offset;
				maxWidth = right - left;
			} else {
...
module.exports = function(Chart) {
	...
	function fitWithPointLabels(scale) {
		 ...
		 * on each side, removing that from the size, halving it and adding the left x protrusion halfWidth.
		 *
		...
		// Get maximum radius of the polygon. Either half the halfLength (minus the text halfWidth) or half the halfWidth.
 ...
 * extremity (left + halfWidth or top + halfLength). Note that it would be more optimized to directly
...
module.exports = function() {
	...
	var TimeScale = Scale.extend({
		...
		/**
		 * Crude approximation of what the label halfWidth might be
		 * @private
		 */
